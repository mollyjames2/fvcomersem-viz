...
if plot_kde:
    ...
    kde_stoichiometry_2x2(
        ...
        months=[6, 7, 8],
        years=None,  # Jun-Aug across all years
        base_dir=BASE_DIR,
        figures_root=FIG_DIR,
        groups=GROUPS,
        min_samples=200,
...
#           | |    / _ \| '_ ` _ \| '_ ` _ \| | | | '_ \| | __| | | |
#           | |___| (_) | | | | | | | | | | | |_| | | | | | |_| |_| |
#            \_____\___/|_| |_| |_|_| |_| |_|\__,_|_|_|_|_|\__|\__, |
#            / ____|                              (_) | (_)     __/ |
#           | |     ___  _ __ ___  _ __   ___  ___ _| |_ _  ___|___/_
#           | |    / _ \| '_ ` _ \| '_ \ / _ \/ __| | __| |/ _ \| '_ \
#           | |___| (_) | | | | | | |_) | (_) \__ \ | |_| | (_) | | | |
#            \_____\___/|_| |_| |_| .__/ \___/|___/_|\__|_|\___/|_| |_|
#                                 | |
#                                 |_|
# Composition (Phyto vs Zoo)
# --------------------------
# Show the FRACTIONAL makeup of phytoplankton and zooplankton groups
# after applying time filters, spatial scope (domain / region / station),
# and depth selection. Each bar sums to 1.0 and is split by group variables.

# Functions:
#   composition_surface_bottom(...)
#     One figure • FOUR bars:
...
#     Depth-avg: tries select_depth("depth_avg") (weighted if supported),
#                else unweighted mean over "siglay".
#
#   composition_at_depth_single(z_level, tol=0.75, ...)
#     One figure • TWO bars at absolute depth:
#       [Phyto (z)] [Zoo (z)]
#     Depth: nearest layer to {"z_m": z_level} within ±tol (meters; negative down).

# Inputs (common):
#   ds: xr.Dataset
#   time filters: months / years / start_date / end_date
#   scope: domain | region=(name, spec) | station=(name, lat, lon)
#   groups: phyto_vars / zoo_vars (only variables present are used)
#   figure: dpi, figsize, verbose
#   saving: base_dir, figures_root

# How fractions are computed:
#   1) Resolve available variables (tolerant naming).
#   2) Pool each variable over remaining dims (sum over time/space/depth).
#   3) Normalize by group total → bar segments sum to 1.0.
#      - Missing vars → NaN → treated as 0 if any finite data exist.
#      - If total ≤ 0 or all-NaN → bar effectively empty.

# Output path:
#   FIG_DIR/<basename(BASE_DIR)>/composition/
# Filenames encode mode (SurfBottom / DepthAvg / zXXm), scope, and time window.
# Titles include Who (Domain/Region/Station) and When (time label).

# Interpretation:
# - Surface vs bottom contrasts reveal vertical niche partitioning.
# - Depth-avg highlights bulk community structure over the column.
# - Fixed z shows conditions at ecologically relevant depths (e.g., DCM).
# - Shifts in fractions indicate relative dominance, not absolute biomass.

# Tips & caveats:
# - Use mutually exclusive group lists so fractions are interpretable.
# - Pair with totals (biomass maps/timeseries) when magnitude matters.
# - Absolute depth requires select_depth to support {"z_m": ..., "tol": ...}.
# - If "depth_avg" weighting is unavailable, fallback is unweighted over "siglay".
#
